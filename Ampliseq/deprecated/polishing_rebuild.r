...
#setwd("F:\\MRD\\卵巢癌WES潘主任\\真固\\多重设计\\batch2\\PR\\")
...
FitModelbyType <- function ( df, nctcols, scale=10000, remove_zero=T ) {
  ...
  for ( i in rownames(df) ) {
    ...
    tryCatch({
      ...
      warning = function(war){ b<-1 }, error = function(err){ b<-1 }) 
...
# main function
Main <- function (nct_file, test_file, outfile) {
  ...
  overall_bgaf <- OverallVAFbyGaussian(NCTdf, NCTcols, remove_zero=T, pvalue=0.05, maxvaf=0.01)
  NCTmodel <- ModelCalculateVAFCutoff(NCTfit, pvalue=0.05, vaf_cutoff=overall_bgaf)
  ...
  write.table(NCTmodel, paste0(outfile,'.model_fit.tsv'), quote=F, sep="\t")
  write.table(TESTresult, paste0(outfile,'.polishing.tsv'), quote=F, sep="\t")
...
option_list = list(
  make_option("--nctvaf", type="character", default=NULL, help="nct vaf tsv file, first column is mutation_id, other columns are VAF of different NCT samples", metavar="character"),
  make_option("--nctdp", type="character", default=NULL, help="nct dp tsv file, first column is mutation_id, other columns are depth of different NCT samples", metavar="character"),
  make_option("--testvaf", type="character", default=NULL, help="test vaf tsv file, same format as --nctvaf file", metavar="character"),
  make_option("--testdp", type="character", default=NULL, help="test dp tsv file, same format as --nctdp file", metavar="character"),
  make_option("--plot", type="character", default=NULL, help="plot result by each sample or all sample, choose from [each, all],\nif do not want plot, don't specify this option", metavar="character"),
  make_option("--ylim", type="character", default=0.005, help="y-axis limit value for plot, default 0.005", metavar="character"),
...
results <- Main(opt$nctvaf, opt$testvaf, opt$out)
if (is.null(opt$plot)) {
